...
func byteReverseHint(_ *big.Int, inputs []*big.Int, outputs []*big.Int) error {
	...
	// Convert LE to big.Int by reversing to get BE
	be := make([]byte, 32)
	for i := 0; i < 32; i++ {
		be[i] = reversed[31-i]
	}
	...
	// Apply mod reduction (critical! from_le_bytes_mod_order reduces mod p)
...
// 1. challenge_bytes(&mut buf) - fills 16 bytes from hash state (LE)
// 2. buf.reverse() - reverse the 16 bytes
// 3. JF::from_bytes(&buf) = Fr::from_le_bytes_mod_order(&buf) - interpret as LE
//
// If original LE bytes are [b0, b1, ..., b15]:
// - After reverse: [b15, b14, ..., b0]
// - from_le_bytes_mod_order([b15, b14, ..., b0]) = b15 + b14*256 + ... + b0*256^15
//
// This is equivalent to interpreting the original LE bytes [b0,...,b15] as big-endian.
// So from_le_bytes_mod_order(reverse(x)) = from_be_bytes_mod_order(x)
//
...
func truncate128Hint(_ *big.Int, inputs []*big.Int, outputs []*big.Int) error {
	...
	// Rust: reverse 16 bytes, then from_le_bytes_mod_order
	// from_le_bytes_mod_order(reverse([b0,...,b15])) = from_le([b15,...,b0])
	// = b15 + b14*256 + ... + b0*256^15
	//
	// This equals from_be_bytes_mod_order([b0,...,b15])
	// Go's SetBytes interprets as big-endian, so we pass le16 directly
	// SetBytes([b0,...,b15]) = b0*256^15 + b1*256^14 + ... + b15
	//
	// Wait, that's NOT the same! Let me reconsider...
	// reverse([b0,...,b15]) = [b15,...,b0]
	// from_le([b15,...,b0]) = b15*256^0 + b14*256^1 + ... + b0*256^15
	// = b15 + b14*256 + ... + b0*256^15
	//
	// Go SetBytes([b0,...,b15]) = b0*256^15 + b1*256^14 + ... + b15*256^0
	// = b0*256^15 + ... + b15
	//
	// These are different! We need to match Rust exactly.
	// To get b15 + b14*256 + ... + b0*256^15, we should pass le16 directly to SetBytes
	// because SetBytes treats first byte as MSB:
	// SetBytes([b0,b1,...,b15]) = b0*256^15 + b1*256^14 + ... + b15
	//
	// But Rust wants: b15 + b14*256 + ... + b0*256^15
	//
	// So the correct Go approach is to NOT reverse le16, just pass it to SetBytes.
	// This gives b0*256^15 + ... + b15, but we need b15 + ... + b0*256^15.
	//
	// Actually, these ARE the same value written differently!
	// b15 + b14*256 + b13*256^2 + ... + b0*256^15 = b0*256^15 + b1*256^14 + ... + b15
	// when we expand both sides. Wait no...
	//
	// Let me use concrete numbers: le16 = [0x01, 0x02]
	// Rust: reverse → [0x02, 0x01], from_le([0x02, 0x01]) = 0x02 + 0x01*256 = 0x0102 = 258
	// Go SetBytes([0x01, 0x02]) = 0x01*256 + 0x02 = 0x0102 = 258 ✓
	//
	// So Go SetBytes(le16) without reversing gives the same result!
	outputs[0] = new(big.Int).SetBytes(le16)
	return nil
}
