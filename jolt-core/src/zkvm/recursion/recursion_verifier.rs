...
//!
//! The `verify_sumchecks` method is fully generic over `F: JoltField` and
...
impl RecursionVerifier<Fq> {
    ...
    pub fn verify<T: Transcript, PCS: CommitmentScheme<Field = Fq>>(
        ...
        // Run all sumcheck stages (generic over field type and accumulator)
        self.verify_sumchecks::<Fq, T, VerifierOpeningAccumulator<Fq>>(
            &proof.stage1_proof,
            &proof.stage2_proof,
            proof.stage3_m_eval,
            &proof.stage4_proof,
    ...
    /// Verify all sumcheck stages (1-5) - generic over field type F.
    ///
    ...
    #[tracing::instrument(skip_all, name = "RecursionVerifier::verify_sumchecks")]
    pub fn verify_sumchecks<F: JoltField + allocative::Allocative, T: Transcript, A: OpeningAccumulator<F>>(
        &self,
        ...
        let _r_stage1_packed = tracing::info_span!("verify_recursion_stage1").in_scope(|| {
            ...
            self.verify_stage1::<F, T, A>(stage1_proof, transcript, accumulator)
        })?;
        drop(_cycle_stage1);
        ...
        let r_x = tracing::info_span!("verify_recursion_stage2").in_scope(|| {
            ...
            self.verify_stage2::<F, T, A>(stage2_proof, transcript, accumulator)
        })?;
        drop(_cycle_stage2);
        ...
        let r_s = tracing::info_span!("verify_recursion_stage3").in_scope(|| {
            ...
            self.verify_stage3::<F, T, A>(transcript, accumulator, &r_x, stage3_m_eval)
        })?;
        drop(_cycle_stage3);
    ...
    /// Verify Stage 1: Packed GT exp sumcheck - generic over F
    #[tracing::instrument(skip_all, name = "RecursionVerifier::verify_stage1")]
    fn verify_stage1<F: JoltField, T: Transcript, A: OpeningAccumulator<F>>(
        &self,
    ...
    fn verify_stage2<F: JoltField + allocative::Allocative, T: Transcript, A: OpeningAccumulator<F>>(
        &self,
        proof: &crate::subprotocols::sumcheck::SumcheckInstanceProof<F, T>,
        transcript: &mut T,
        accumulator: &mut A,
    ...
    /// Verify Stage 3: Direct evaluation protocol - generic over F.
    #[tracing::instrument(skip_all, name = "RecursionVerifier::verify_stage3")]
    fn verify_stage3<F: JoltField, T: Transcript, A: OpeningAccumulator<F>>(
        &self,
        transcript: &mut T,
        accumulator: &mut A,
        r_x: &[F::Challenge],
    ...
    /// Verify Stage 4: Jagged transform sumcheck - generic over F.
    #[tracing::instrument(skip_all, name = "RecursionVerifier::verify_stage4")]
    fn verify_stage4<F: JoltField, T: Transcript, A: OpeningAccumulator<F>>(
        &self,
        stage4_proof: &crate::subprotocols::sumcheck::SumcheckInstanceProof<F, T>,
        stage5_proof: &super::stage5::jagged_assist::JaggedAssistProof<F, T>,
        transcript: &mut T,
    ...
    /// Verify Stage 5: Jagged assist - generic over F.
    #[tracing::instrument(skip_all, name = "RecursionVerifier::verify_stage5")]
    fn verify_stage5<F: JoltField, T: Transcript, A: OpeningAccumulator<F>>(
        &self,
        stage5_proof: &super::stage5::jagged_assist::JaggedAssistProof<F, T>,
        transcript: &mut T,
        accumulator: &mut A,
